...
queryDataNew1 <- read_csv("/home/kdaily/Projects/LINCS/annualreport/syn2862345_report_2018-10-23.csv") %>% select(one_of(keep_cols))

queryDataNew2 <- read_csv("/home/kdaily/Projects/LINCS/annualreport/syn2862345_report_2018-04-04.csv") %>% select(one_of(keep_cols))

queryDataNew3 <- read_csv("/home/kdaily/Projects/LINCS/annualreport/syn2862345-20180701-20190408.csv") %>% 
  mutate(userid=userId) %>% 
  select(one_of(keep_cols)) 

queryDataNew4 <- read_csv("/home/kdaily/Projects/LINCS/annualreport/syn2862345-20181001-20190930.csv")%>% 
  mutate(userid=userId) %>% 
  select(one_of(keep_cols)) 
...
queryDataOld <- read_csv("/home/kdaily/Projects/LINCS/annualreport/syn2862345_20170407_26months-simplified.csv") %>% 
  select(one_of(keep_cols)) %>% 
  mutate(userId=as.character(userId),
         quarter=lubridate::quarter(DATE),
         year=lubridate::year(DATE),
...
```{r}
pageview_ga <- readr::read_csv("/home/kdaily/Projects/LINCS/annualreport/Analytics Synapse MEP-LINCS 20161001-20190930.csv", comment = "#") %>% 
